...
class BBA_3rd_SEM extends StatefulWidget {
  const BBA_3rd_SEM({Key? key}) : super(key: key);

  @override
  State<BBA_3rd_SEM> createState() => _BBA_3rd_SEMState();
}
...
final List<String> pdfUrls = [
  "https://vcmdrp.tums.ac.ir/files/financial/istgahe_mali/moton_english/financial_management_%5Bwww.accfile.com%5D.pdf",
...
class _BBA_3rd_SEMState extends State<BBA_3rd_SEM> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("BBA 3rd Semester Book"),
      ),
                      ...
                      child: InkWell(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => bba_2ye_1se(pdfUrl: pdfUrls[i]),
                            ),
                          );
                        },
                        child: Padding(
...
class bba_2ye_1se extends StatelessWidget {
  final String pdfUrl;

  const bba_2ye_1se({Key? key, required this.pdfUrl}) : super(key: key);
